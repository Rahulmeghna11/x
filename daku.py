...
TELEGRAM_BOT_TOKEN = '7335032114:AAHYvubBycrdfUA1IfnhlTtPg2eeQ6wQnRE'
ADMIN_USER_ID = 1661744209
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        "*⚠️CONTACT THE OWNER FOR PURCHASE--> @RAHUL_DDOS_B"
    )
...
async def help_command(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        "⚠️CONTACT THE OWNER--> @RAHUL_DDOS_B"
    )
    await context.bot.send_message(chat_id=chat_id, text=message, parse_mode='Markdown')
...
async def plane(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        "😈1 Month = 650 Rs\n"
        "🤑Lifetime = 800 Rs\n"
        "⚠️CONTACT THE ADMIN--> @RAHUL_DDOS_B"
    )
    await context.bot.send_message(chat_id=chat_id, text=message, parse_mode='Markdown')
...
async def daku(update: Update, context: CallbackContext):
    ...
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Usage: /rahul <add|rem> <user_id> <days>*", parse_mode='Markdown')
        return
...
async def attack(update: Update, context: CallbackContext):
    ...
    if int(duration) > 200:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Attack duration cannot exceed 200 seconds.*", parse_mode='Markdown')
        return
    ...
    await context.bot.send_message(chat_id=chat_id, text=(
        ...
        f"*🙏 Please Send Feedback To Admin--> @RAHUL_DDOS_B"
    ), parse_mode='Markdown')
